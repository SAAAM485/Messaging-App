...
const conversationService = {
    ...
    getConversationById: async (conversationId) => {
        const { conversationId: convId } = convIdSchema.parse(conversationId);
    ...
    addUserToConversation: async (conversationId, userId) => {
        const { conversationId: convId } = convIdSchema.parse({
            conversationId,
        });
        const { userId: uId } = userIdSchema.parse({ userId });
        const conv = await getConversationById(convId);
        if (!conv) throw new ConversationError("Conversation not found");
        if (!conv.isGroup)
    ...
    leaveConversation: async (convPartId) => {
        const { convPartId: partId } = convPartIdSchema.parse({ convPartId });
        return removeParticipant(partId);
    },
